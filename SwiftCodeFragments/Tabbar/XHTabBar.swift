...
func ColorRandom() -> UIColor {
    ...
    return ColorRGB(r: CGFloat(arc4random()%255), g: CGFloat(arc4random()%255), b: CGFloat(arc4random()%255))
}
/**
 *  屏幕宽度
 */
...
public class XHTabBar:UITabBarController {
    ...
    //MARK: - 懒加载
    public lazy var cusTabbar: UIView = {
        ...
        let view = UIView(frame: CGRect.init(x: x, y: y, width: width, height: height))
    ...
    private func addTabBarButton()
    {
        ...
        for i in 0..<num {
            ...
            let  h = tabBarHeight
            let button = XHTabBarButton(frame: CGRect.init(x: x, y: y, width: w, height: h))
            ...
            button.setImage(UIImage.init(named:self.imageArray[i]), for: UIControlState.normal)
            ...
            //角标
            let numLabel = UILabel(frame: CGRect.init(x: button.frame.size.width/2.0 + 6, y: 3, width: numMarkD, height: numMarkD))
    ...
    @objc private func buttonAction(button: UIButton) {
        ...
        self.showControllerIndex(index: index)
    }
...
extension XHTabBar{
    ...
    public func showBadgeMark(badge: Int, index: Int) {
        ...
        let numLabel = (self.view.viewWithTag(1020+index) as? UILabel)!
        numLabel.isHidden = false
        var nFrame = numLabel.frame
        if badge <= 0 {
            //隐藏角标
            self.hideMarkIndex(index: index)
